...
enum { SchemeNorm, SchemeSel, SchemeTitle, SchemeSelTitle, SchemeSeparator }; /* color schemes */
enum {
	NetSupported,
	NetWMName,
	NetWMState,
	...
	if (XineramaIsActive(dpy)) {
		...
		for (i = 0, m = mons; i < nn && m; m = m->next, i++)
			if (i >= n || unique[i].x_org != m->mx || unique[i].y_org != m->my ||
				...
				if (unique[i].height > unique[i].width) m->nmaster = nmasterportrait;
				updatebarpos(m);
			}
...
void updatesizehints(Client *c) {
	...
	if (!XGetWMNormalHints(dpy, c->win, &size, &msize)) /* size is uninitialized, ensure that
